...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = `You are the AI Academic Planning Engine. Analyze the student's current workload and provide a proactive recommendation.
Context:
  ...
  "workloadWarning": "A short sentence advising on their backlog (e.g., 'You have 3 heavy tasks due tomorrow. Start now.')"
    ...
    if (groqRes.ok) {
      ...
      return NextResponse.json({
        ...
        workloadWarning: "Unable to connect to AI engine."
      });
    }
