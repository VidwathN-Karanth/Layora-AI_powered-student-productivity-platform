  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-white/5 pb-4">
        <h2 className="text-xl font-mono font-bold tracking-wide">RESOURCE_VAULT</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="glass-card rounded-2xl p-5 space-y-4">
            <h3 className="text-xs font-mono font-bold text-purple-400 border-b border-white/5 pb-2">UPLOAD_RESOURCE</h3>
            ...
            {subjects.length === 0 ? (
              ...
              <form onSubmit={handleUpload} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-mono text-white/50 mb-1">TARGET_SUBJECT</label>
                  <select
                ...
                <div>
                  <label className="block text-[10px] font-mono text-white/50 mb-1">DOCUMENT_NAME</label>
                  <input
                    type="text"
                    required
                    value={fileName}
                    onChange={(e) => setFileName(e.target.value)}
                    placeholder="Calculus_Cheat_Sheet"
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-mono text-white/50 mb-1">FILE_FORMAT</label>
                  <select
                    value={fileType}
                ...
                >
                  <UploadCloud className="w-4 h-4" /> COMMIT_UPLOAD
                </button>
              </form>
            )}
          </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white/2 p-3 rounded-xl border border-white/5">
            <span className="text-[10px] font-mono text-white/50">VAULT_INDEX_BY_SUBJECT</span>
          </div>
