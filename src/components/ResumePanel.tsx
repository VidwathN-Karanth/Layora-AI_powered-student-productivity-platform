...
import {
  FileText, UploadCloud, Link2, Trash, ExternalLink, Loader2, Check, Lock,
} from 'lucide-react';
...
export default function ResumePanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Could not load your CV.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const remove = async () => {
    if (!confirm('Remove your CV? Staff will no longer be able to see it.')) return;
    setBusy(true);
    setError('');
    try {
  ...
  return (
    <div className="glass-card rounded-2xl p-5 space-y-4">
        ...
        <div className="space-y-3">
          <div className="bg-surface-container border border-outline-variant rounded-xl p-3.5 flex items-center gap-3">
            ...
            <button
              onClick={remove}
              disabled={busy}
              className="p-2 rounded-lg border border-outline-variant hover:border-red-400 text-outline hover:text-red-400 transition cursor-pointer shrink-0 disabled:opacity-40"
          ...
          <p className="text-[9px] font-mono text-outline">
            Uploading again replaces this one.
          </p>
        </div>
      ) : null}
      ...
      <form onSubmit={save} className="space-y-3">
        ...
        {mode === 'upload' ? (
          <div>
            <input
              ...
              className="w-full text-[10px] font-mono text-outline file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border file:border-outline-variant file:bg-surface-container file:text-on-surface file:text-[10px] file:font-bold file:cursor-pointer"
            />
            <p className="text-[9px] font-mono text-outline mt-1.5">
              PDF up to 4.5 MB. Saved to your own Google Drive.
            </p>
          </div>
        ) : (
          <div>
        ...
        >
          {busy ? (<><Loader2 className="w-3.5 h-3.5 animate-spin" /> SAVING...</>) : resume ? 'REPLACE CV' : 'SAVE CV'}
        </button>
      </form>
    </div>
  );
